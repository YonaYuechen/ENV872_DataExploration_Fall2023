...
item_name <- c('Source', "Date", "Filename")
value <- c("EPA Air Quality System (AQS)", "2018-2019", "EPAair_O3_PM25_NC1819_Processed.csv")
EPA_air <- cbind(item_name,value)

knitr::kable(EPA_air, caption = 'EPA Air Quality', col.names = c('Item Name', 'Value'), align = c('r', 'l'))
```
...
Recorded Ozone by Month (2018)
```{r figure 1, echo=FALSE, fig.cap='Monthly Ozone, 2018', fig.height=3.5, message=FALSE, warning=FALSE, fig.align = 'left'}
ozoneMon18 <- ggplot(data = filter(epa_data, Year == '2018'), 
                     aes(x = factor(Month,levels = 1:12, labels = month.abb), 
                         y = Ozone)) + geom_boxplot() + 
  labs(x = 'Month', y = expression(paste("Concentration ( ", mu, "g/", m^3, ')')))
print(ozoneMon18)
```

\
Recorded Ozone by Month (2019)
```{r figure 2, echo=FALSE, fig.cap='Monthly Ozone, 2019', fig.height=3.5, message=FALSE, warning=FALSE, fig.align = 'left'}
ozoneMon19 <- ggplot(data = filter(epa_data, Year == '2019'), 
                     aes(x = factor(Month,levels = 1:12, labels = month.abb), 
                         y = Ozone)) + geom_boxplot() + 
  labs(x = 'Month', y = expression(paste("Concentration ( ", mu, "g/", m^3, ')')))
print(ozoneMon19)
...
## Git Repository
https://github.com/YonaYuechen/ENV872_DataExploration_Fall2023