...
# 4
GaringerOzone_processed <- GaringerOzone %>% select(Date, Daily.Max.8.hour.Ozone.Concentration, DAILY_AQI_VALUE)
...
```{r}
#7
ggplot(GaringerOzone, aes(x = Date, y = Daily.Max.8.hour.Ozone.Concentration)) + geom_line() + geom_smooth(method = lm, se = FALSE) + ylab("Concentration (ppm)") + YH_Theme
```
...
```{r}
#9
GaringerOzone.monthly <- GaringerOzone %>% mutate(Year = year(Date), Month = month(Date)) %>% group_by(Year, Month) %>% summarise(mean_ozone_month = Daily.Max.8.hour.Ozone.Concentration)

GaringerOzone.monthly <- mutate(GaringerOzone.monthly, Date = make_date(Year, Month, 1))
```
