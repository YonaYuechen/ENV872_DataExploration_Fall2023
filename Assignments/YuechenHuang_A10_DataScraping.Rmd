...
#2 
Durham_LWSP_web <- read_html("https://www.ncwater.org/WUDC/app/LWSP/report.php?pwsid=03-32-010&year=2022")
Durham_LWSP_web
```
...
Scrape.fun <- function(the_PWSID,the_year){
  ...
  Month_wrong_order <- the_website %>% html_nodes(".fancy-table:nth-child(31) tr+ tr th") %>% html_text()
  df_raw <- data.frame("Month" = Month_wrong_order,
...
#7 
new_df_using_func <- Scrape.fun("03-32-010",2015)
ggplot(new_df_using_func,aes(x=Date,y=Maximum_Day_Use)) + 
  geom_line() + 
  geom_smooth(method="loess",se=FALSE) +
