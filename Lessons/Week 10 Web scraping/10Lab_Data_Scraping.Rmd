...
ctrl + U ---> can see the html language of a webpage
...
# uv : function
# search_site_no, period, format: arguments that the function needs
...
gage_data2 <- read.table(theURL,skip = 30,header=FALSE,sep='\t',stringsAsFactors = T)
...
#Load in data using the package's "readNWISdv" function; dv:daily value
...
nc_pop <- 
  get_acs(geography = "county",
          ...
          state = "NC",
          geometry = TRUE)
# acs: american comunity service ()
# geometry: return a spatial data frame; if FALSE, normal data frame
...
## Scraping Exercises
First see whether have download options. If not, try API. APIs are great, when they are available. But if not, we can still scrape data from a web site. Contact the person before scraping. 
...
energy_data <- data.frame(
  "State" = the_states,
  ...
  "Capacity" = as.numetric(gsub(",","",the_capacity)),
