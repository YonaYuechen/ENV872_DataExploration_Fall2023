...
matrix1[4, ] #
matrix1[ , 3] #
matrix1[c(12, 14)] # number at position 12 and 14
matrix1[c(12:14)] # 
matrix1[2:4, 1:3] # row from 2rd to 4th, column 1 to 3
...
dataframe1 <- data.frame(d,e,f) # Another approach is to factor and cbind() with matrix
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # names can be useful
...
```{r DataExercise-1c}
df_temp <- data.frame('HighTemF' = high, 'LowTemF' = )
# name(df_temp) <- c('', '', '')
```
...
date_obj_today <- as.Date(str_today, Format = '%d-%m-%y')
