...
library(lubridate)
NTL.phys.data <- read.csv("./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv", stringsAsFactors = TRUE)  # stringsAsFactors = TRUE very important when importing data
...
class(NTL.phys.data$sampledate) # always check if date are date type of data
...
```{r, results = "hide"}
vignette("dplyr")  ## can find more infor here
## Rows:
...
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))  # %in% useful when having multiple OR together
...
```{r selecting columns}
NTL.phys.data.temps <- select(NTL.phys.data, lakename, sampledate:temperature_C) # because sampledata and temperature_c are next to each other can just use :
...
NTL.phys.data.PeterPaul1 <- select(NTL.phys.data.PeterPaul1, lakeid:daynum, month, sampledate:comments)  # put month between other column
...
interval(first(NTL.phys.data.PeterPaul1$sampledate), last(NTL.phys.data.PeterPaul1$sampledate)) # same as [1] and [21613]; will return the first value --- the last value
```
