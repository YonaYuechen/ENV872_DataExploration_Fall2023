...
NTL.nutrient.data.PeterPaul <- 
  ...
  drop_na(depth) # drop all depth NA values
...
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration)) # remove all values that have n/a concentration
...
LitterFiles  # list.files() can list all the files that we want in a folder 

Litter <- LitterFiles %>%
  plyr::ldply(read.csv)  ## read all files and join all files? But how does ldply work?
```
...
NTL.PeterPaul.summaries <- 
  ...
  summarise(meantemp = mean(temperature_C), 
            ...
            sdTN = sd(tn_ug), 
            meanTP = mean(tp_ug), 
            sdTP = sd(tp_ug))  # summarise can be very useful! NOT summary()!
