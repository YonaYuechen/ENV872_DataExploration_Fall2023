...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
mytheme <- theme_classic(base_size = 14) # change font size
+
  theme(axis.text = element_text(color = "black"), # axis text color to black
        legend.position = "top") #legend show on the top #alternative: legend.position + legend.justification
...
theme_set(mytheme) # set the theme as all theme for plots generated
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  ...
  geom_point(alpha = 0.5, size = 1.5) + # alpha means transparancy
  ...
  ylab(expression("O"[3]* " AQI Value")) + # if have [3]* need to have expression()
...
# place your additional edits here
  ...
  #scale_color_brewer(palette = "YlGnBu") + # discrete data
