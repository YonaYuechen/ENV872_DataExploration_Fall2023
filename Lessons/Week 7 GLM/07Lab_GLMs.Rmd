...
```{r}
#Exercise 1:
density_plot <- ggplot(EPAair, aes(x = Year, y = Ozone), geom_density(alpha = 0.4))
density_plot

# correct exercise 1
density_plot <- ggplot(EPAair, aes(x = Ozone, fill = as.factor(Year)) + geom_density(alpha = 0.4)) + 
  scale_fill_manual(values = 'darkgreen', 'darkpurple') + labs(x = 'Ozone AQI value', y = "Density", label = '')
density_plot
...
stat_summary(geom = 'text',
             ...
             vjust = -1,
             size = 4,
             lable = c('a', 'b'))  # add the label on the top of the graph
...
lr_PM25_Ozone <- lm(data = EPAair, PM2.5 ~ Ozone)
summary(lr_PM25_Ozone)  ## don't forget
...
library(corrplot)
library(dplyr)
EPAair.subset <- EPAir %>% select(mean_Lat:Ozone) %>% summarise()
