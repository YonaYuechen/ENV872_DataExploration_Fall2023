...
summary(is.na(Rainfall_BR_processed)) # or can just use summary() if have NA will have a column called NA after Max
summary(is.na(Temp_BR_processed))

#seq1 <- seq(from = , to= , by)
#left_join(seq1, dataset) # so that have a continuous dataset that can be check NA
...
plot_rain <- ggplot(Rainfall_BR_processed, aes(x = Date, y = Rainfall)) + 
  geom_line() + mytheme + geom_smooth()
plot_rain
# don't have a trend (should be increasing overall) but have a pattern (up and downs)

plot_temp <- ggplot(Temp_BR_processed, aes(x = Date, y = Temperature_C)) + 
  geom_line() + mytheme + geom_smooth(method = lm)
plot_temp
...
#Rainfall_ts <- ts(Rainfall_BR_processed, start = c(1901,1), frequency = 12) is wrong! because we need to specify the column (only one col each time)
# always start = c(year, month, date)


# can also use month(first(Temp_BR_processed$Temperature_C)) for start value 
...
# to see better trend ---> only see dry seasons or wet seasons (break it down)

Temp_decomp <- stl(Temp_ts, s.window = "periodic")
plot(Temp_decomp)
# should not see any trend in the bottom plot(reminder)
