...
into R with `sf` - Spatial aggregation with `group_by` and `summarize`
or `st_union` - Visualizing multiple datasets - Changing CRS with
...
The typical set up procedure: import libraries and confirm the working
directory
...
#
st_crs(counties_sf)  # coornidate reference system
...
#Reveal the columns
colnames(huc8_sf) # huc: Hydrologic Unit Code
...
mapView(huc8_sf, zcol = "ACRES") # zcol = ?
...
upperNeuse <- st_read(here('Data/Spatial/NCHUC8.shp')) %>% filter(SUBBASIN == "Upper Neuse")


#Create a map
mapView(huc8_sf, col.region = "orange", layer.name = "HUC 8") + 
  mapView(upperNeuse, col.region = "purple", layer.name = "Upper Neuse") ## purple on the top

ggplot() + geom_sf(data = huc8_sf, fill = "orange") + 
  geom_sf(data = upperNeuse, fill = "purple")
# Tips: don't put data in ggplot() and geom_sf need to be the same
...
[link](https://duke.is/8/jpzm). - Click the "I want to use this" button
at the bottom left of the page. - Expand the "View API Resources" to
view the link to the resource in GeoJSON format. - Copy the link to the
GeoJSON option.
...
huc2_sf <- huc8_sf %>% group_by(DWQ_Basin) %>% 
...
neuse_intersect_2 <- counties_sf_utm %>% st_filter(neuse_sf)
...
Buffering features - Union many features to a single multi-part features
- Compute convex hulls from multi-part features - Compute Voronoi
polygons from multi-part features
